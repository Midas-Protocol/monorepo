...
export const formatPrice = (price: bigint, token: { address: string; decimals: number }): string => {
  return formatEther(price / BigInt(10) ** BigInt(18 - token.decimals));
};

export const isInverted = (address: string, wNativeAddress: string) => BigInt(address) > BigInt(wNativeAddress);
